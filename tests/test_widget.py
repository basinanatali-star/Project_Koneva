...
def test_mask_account_card(cart_number: str, result: str) -> None:
    ...
    with pytest.raises(ValueError) as err:
        ...
        assert str(err.value) == ValueError("Проверьте правильность ввода номера счета. Он должен содержать 16 цифр!")
    with pytest.raises(ValueError) as err:
        mask_account_card("Счет 123456789012345678")
        assert str(err.value) == ValueError("Проверьте правильность ввода номера счета. Он должен содержать 20 цифр!")
    with pytest.raises(ValueError) as err:
